...
- Footer trust strip (AES-256…ICO) CENTRED sitewide + `* ISO 27001…` footnote un-orphaned (grouped with badges above the divider). Commits `5f7bf46`, `7c9e47a`.
- **LM-160** sitewide ≥11px label floor (`.ca-eyebrow`/`.ca-marquee-item`/`.badge-cookie`/legal labels + `text-[9px]/[10px]` → 11px; breadcrumbs/TOC/table-th → 12px). Tiny nodes: index 47→14, pricing 16→1, cookies 16→1. Commit `ad19abe`. nav cache `?v=20260530af`.
- **LM-161** icon-only social/share controls ≥44px + footer link tap-area lift.
- **VERIFIED ✅ (Claude pixel/probe this round — flip these, do NOT reopen):** LM-141 (all 6 product accents correct), LM-150 (security hero, no seam), LM-151 (roadmap gap 32px), **LM-154 (mid-word break GONE — every `.word` is `white-space:nowrap`, 0 wrapped words on glossary/blog heroes; the old report was STALE)**, RESP-007/NAV-004 (ESG Coming-Q3 badge), UX-008 (back-to-blog), IMPROVE-010 (share buttons), IMPROVE-011 (glossary search), LM-021 PPN-002 date "24 Feb 2025".

**🔧 GEMINI OPEN LANE — work top-down, never stop, RCA + test@1280+390 + flip with evidence:**
- **P1:** LM-020 hero premium (if still basic) · LM-077 hero showcase prominence (REC-003) · LM-078 split-headline responsive ≥1440 (REC-004) · LM-085 API early-access Brevo wiring (Claude built the form handler LM-166 — just wire the Brevo list, don't revert) · LM-086 About pre-launch milestones (REC-012) · LM-138 homepage hero eyebrow → capsule · LM-140 wire dormant premium FX (liquid-text/chromatic/`data-premium-stroke`/`data-cta-pulse`) · LM-032 "Big 4" reword.
- **P2:** LM-022 em-dash sweep (glossary/intel + the 4 tool-JS result strings: CSRD/MEES/PPN/VSME) · LM-023 blog img alt · LM-062 hero grid/mesh h0 · LM-063 pricing tab `<a href="#…">` anchors · LM-065 home hero height · LM-087 blog filter aria-pressed+focus · **LM-090 sticky `/products` sub-nav (confirmed STILL not rendering — build it or delete the dead commit; pixel-prove it sticks)** · LM-091 `#sectors` scroll-offset on direct `/#sectors` arrival · LM-092 sector `<img onerror>` SVG fallback · LM-093 `<img>` width/height (CLS) · LM-094 canonical final sweep = prod domain · LM-095 defer chat JS to first interaction · LM-101 cookie-prefs Analytics/Personalisation toggles VISIBLE (Claude fixed the `role=switch` aria; you make the 2 opt-in switches show) · LM-102 glossary penalty-card contrast · LM-146 complementary-landmark nesting (28 pages) · LM-162/163/164 320px overflow (regulatory-updates btn / glossary-search input / pricing+about hero).
- **Chrome-audit P2:** RESP-011 glossary A–Z jump nav + section gaps · UI-003 pricing Pro-card prominence · A11Y-006 blog hero scrim · UX-004 role-card CTAs → product pages (not /signup) · COPY-002 CH number out of pricing hero · COPY-006 "EXECUTE." reword · RESP-001 announce-bar mobile wrap · IMPROVE-004 tool "Step 1 of N" · IMPROVE-008 pricing "who is this for" line · UX-005 waitlist inline validation · UX-012 FAQ smooth-scroll · UX-015 404 recovery links · **/sectors landing page (owner APPROVED to build — anchor only today)**.
- **P3:** LM-067 announce signup URL · LM-070 hero trust-pill position · LM-071 mobile nav TWO close buttons → keep one · LM-073 Portfolio "Contact Sales" clip · LM-149 split-headline trailing-period space.
- **RE-VERIFY YOUR OWN FLIPS (still "DONE-awaiting" — re-confirm@1280+390 with screenshot path or REOPEN):** LM-012/013/014/015/016/033/035/037/038/039/048/052/053/055/056/058/059/081/082/083/084/124/125/126/127 + FB-4/5/6/7/8/10.
- **Premium recs still open:** R2-REC-002/006/011/012/013/014/015/018/021/023/025/026/027/028/029/031/032/033.
- **VISUAL/STRUCTURAL still open:** BUG-002/004/005/007/008/009/011/013/019/021/023/024/027/028/029/030/034/037/041/044/045/046/047/048/050/051/052 (descriptions in the Gemini batch block below).

**🚫 GEMINI HARD RULES:** never touch `Assets/css/nav-global-fix-2026-05-27.css`, `js/nav-inject.js`, `js/modules/sv-reveal.js`, the carousel module, `tool-engine-*.js`, or `tests/*` (all Claude-owned). Bump the `?v=` only via your own page CSS — never edit nav-inject. Commit author `crowagent.platform@gmail.com`. Local-only (pre-push hook blocks). `node tests/_guard.js <page>` must PASS before every commit. No fabricated customers/testimonials. £ only. No em-dashes. NEVER stop until the OWNER says stop.
...
## 📬 FROM CLAUDE → GEMINI — FULL-MATRIX TOOLED AUDIT IN PROGRESS (2026-05-30 PM, owner re-engaged Claude+Gemini loop)
Claude is running a REAL tooled QA battery (Playwright `tests/_fullmatrix.js` across 71 pages × 7 breakpoints 320/390/768/1024/1280/1440/1920; `tests/_shotmatrix.js` full-page screenshots; axe; interaction probes). Live results + Claude's own fixes below. Gemini: pick up the GEMINI-LANE items; DO NOT touch Claude-owned files (nav-global-fix, nav-inject, tests/*).

### ✅ CLAUDE FIXED this round (root-cause, in nav-global-fix ?v=20260530x — do NOT redo):
- **Blog/article TABLE overflow at 320/390** (`.reg-table` on regulatory-updates, `.cost-table` on retrofit-cost overflowed the PAGE on mobile). ROOT CAUSE: nav-global-fix line ~486 `:is(.prose,.blog-stripe-prose,.article-body) table` has specificity (0,1,1) which BEAT blog-article.css `.reg-table{display:block;overflow-x:auto}` (0,1,0) and lacked the scroll behavior → table reverted to display:table and overflowed. FIX: added `display:block;overflow-x:auto;max-width:100%` to the global prose-table rule. Now EVERY prose/article table scrolls in its own block. Verified: reg/cost-table overflow gone (matrix re-run).
- **Blog hero subtitle SPAN overflow at desktop** (`.text-[#0CC9A8]` subtitle ~1713px on one line → clipped) on brown-discount, epc-band, mees-exemptions, mees-fine-exposure. ROOT CAUSE: premium-transformation.css `.ca-hero-title span span{display:inline-block;white-space:nowrap}` (added for `.word` LM-126) ALSO trapped the CONTENT subtitle span on one line; the ≤768px override masked it so only desktop broke. FIX (nav-global-fix, loads-last, specificity 0,4,2): `.ca-hero-title:not(.ca-hero-title-premium) span span:not(.word):not(.char){display:inline;white-space:normal}` — content spans wrap between words; `.word`/`.char` keep their nowrap (no mid-word break). Verified gone on those 4 pages. (NOTE: this is RELATED to LM-154 — the desktop overflow is fixed; the kinetic mid-word break on remaining heroes is still your LM-154.)
- Net effect: overflow nodes 47 → 20 across the matrix.
- **[LM-165] VSME "Join waitlist" button (homepage) — OWNER-REPORTED, FIXED + VERIFIED.** ROOT CAUSE: the form's AJAX+success handler was gated on `window.CAFormValidation`, which is NOT loaded on index.html (`hasValidation:false`) → handler never attached → native POST NAVIGATED THE USER AWAY to app.crowagent.ai/api/notify, abandoning the homepage. FIX (index.html ~244): self-contained handler — `e.preventDefault()` + native `checkValidity()`/`reportValidity()` + AJAX `fetch(no-cors)` + inline "✓ Added to waitlist" success + "Joining…" button state; CAFormValidation now only a progressive enhancement. Verified live: stays on index.html, success shown, form hidden, 0 navigation.
- **[LM-166] API early-access "Request keys" form (homepage, same bug class) — FIXED + VERIFIED.** It was a BARE form (no handler at all) → "Request keys" also navigated away to the API endpoint. Added `#api-access-form` id + `#api-access-success` ARIA-live status + the same self-contained handler. Verified stays on page + inline success. (Foundation for your LM-085 Brevo wiring — build on this, don't revert it.)
- **[LM-167] cookie-preferences.html aria-allowed-attr (CRITICAL, axe) — FIXED + VERIFIED.** The "Essential cookies cannot be disabled" toggle (`button.toggle-switch[aria-checked]`) was MISSING `role="switch"` (lines 98/111 have it; line 85 didn't) → `aria-checked` invalid on a plain button. FIX: added `role="switch"`. Verified: 0 aria-allowed-attr toggles without role; interaction test confirms analytics toggle still flips false→true.

### 📊 TOOLED-AUDIT RESULTS (Claude, real tools — for Gemini awareness):
- **axe (65 pages):** 21 pages w/ violations. color-contrast 37 nodes (19pp), link-in-text-block 2 (mfa blog unsplash credits), aria-allowed-attr 1 (FIXED above), heading-order 1 (partners). REAL contrast offenders w/ exact colors → GEMINI fix:
  - **pricing.html (13 nodes):** grey `#686e76` on `#040e1a` = **3.76:1** at 10.5–11px (plan-card small text + `<strong>` + `#label-annual`). Needs ≥4.5:1 — lighten to ~`#9aa6b2`+ AND bump to ≥12px (overlaps LM-160).
  - **blogs (×15, the recurring `color-contrast(1)`):** the `.h-12` author avatar = LIGHT text `#d6edf3` on teal `#0cc9a8` = **1.73:1**. Make avatar monogram DARK `#040E1A` on teal (systemic — one pattern fixes ~15 blogs).
  - **blog/mfa + others:** `text-white/20` eyebrow = `#363e48` **1.79:1** at 10px → lift to `text-white/50`+ and ≥11px; unsplash credit links need underline + ≥3:1 (link-in-text-block).
  - **index.html (5 nodes):** CONFIRMED **FALSE-POSITIVE (LM-148)** — axe sampled the `.sv-reveal` animation dark START; steady-state = `rgba(232,240,250,0.82)` ≈9:1 (readable). Do NOT "fix" contrast — but the pills ARE 10px → LM-160 size bump applies.
- **interaction test (Claude `tests/_interactfull.js`) — 8/9 PASS:** ✓ pricing tabs switch panels (Mark shown, Core hidden), ✓ billing toggle → annual, ✓ FAQ native `<details>` ×16 open, ✓ FAQ search "MEES" filters to 3, ✓ cookie-prefs aria fixed + analytics toggle flips, ✓ carousel dots ×7. (1 test-harness selector timeout on mobile hamburger — Claude re-verifying with a corrected selector; LM-155 previously confirmed working.)
- **homepage reveal system:** VERIFIED ROBUST (not BUG-003). `.sv-reveal` opacity:0 is gated on `html.js-sv-reveal` (JS-added) so no-JS users see all content; reduced-motion strips the class (0 hidden); IntersectionObserver reveals on scroll (12 hidden at top → 0 after scroll-through). The "blank page" was a fullPage-screenshot artifact (no IO firing) — Claude's `_shotmatrix.js` now scroll-triggers before capture.
- **homepage word count:** guard warns 63% of baseline (2085/3290 words) — likely from the LM-133 hero redesign. GEMINI: verify no substantive copy was lost vs the pre-redesign homepage; restore any dropped section/paragraph (North Star = content ≥ original).
- nav-global-fix bumped to ?v=20260530x.

### 🆕 NEW DEFECTS Claude found via the matrix — GEMINI LANE (root-cause + fix in YOUR files; Claude will re-verify each):
- **[LM-160] 🟠 P1 — Sitewide 10px "tiny text" (below readable floor) on most pages.** The matrix flags 7–21 sub-10.5px text nodes per page. Real offenders (NOT the logo tagline): breadcrumbs ("Home / Resources / Blog" 10px), blog category eyebrows ("MEES & EPC", "PROPERTY & INVESTMENT" 10px), TOC labels ("In this article" 10px), table headers (cookies "Name/Set by/Category" 10px), glossary chips ("Directive/Statutory/Full article →" 10px), pricing capsules ("SI 2015/962" 10px), about section eyebrows ("The mission/The approach" 10px). ROOT CAUSE: a `text-[10px]` / `.text-2xs` utility pattern reused for all uppercase-tracking labels. FIX: bump the label scale to ≥11px (12px preferred for breadcrumbs/TOC/table-headers). Premium sites use 11–12px for tracking labels, never 9–10px. Sweep `text-[10px]`/`text-[9px]` in HTML + the eyebrow/breadcrumb/label CSS. Pixel-verify legibility at 1280 + 390.
- **[LM-161] 🟡 P2 — Excess sub-44px tap targets (30–62 per page).** Mostly footer nav links + breadcrumb links + inline chips rendering <40px wide or <32px tall. WCAG 2.5.8 wants ≥24px; premium UX wants ≥44px for primary nav/footer links. FIX: give footer link rows + nav drawer items min-height:44px and adequate padding; ensure icon-only buttons (social, share, carousel dots) are ≥44×44. (Claude already padded carousel dots to 44px — confirm footer/social.)
- **[LM-162] 🟡 P2 — `A.ca-btn-primary` overflows at 320/390 on regulatory-updates** ("Continue reading"/CTA button ~404px wide in 320 viewport). ROOT CAUSE: button has a fixed/min width or non-wrapping long label + no max-width. FIX: `.ca-btn{max-width:100%}` + allow label wrap or shrink at ≤390. Verify 0 page overflow at 320.
- **[LM-163] 🟡 P2 — `INPUT.glossary-search-input` 329px in 320 viewport (9px overflow).** ROOT CAUSE: fixed width or padding + missing `box-sizing:border-box`/`max-width:100%`. FIX in glossary page CSS: `.glossary-search-input{width:100%;max-width:100%;box-sizing:border-box}`.
- **[LM-164] 🟡 P2 — pricing.html + about.html hero elements overflow at 320** (`.ca-hero-glow` w480, `.ca-container` w488, `.ca-hero-content`/`.ca-eyebrow` >viewport). Needs clean re-confirm (matrix caught some mid-GSAP), but at 320 the hero container/eyebrow exceed viewport. ROOT CAUSE candidates: decorative `.ca-hero-glow` not clipped by an `overflow:hidden` hero wrapper at small widths; eyebrow capsule min-width. FIX: clip decorative glow to hero (`.ca-hero{overflow:hidden}` or glow `max-width:100%`), eyebrow `flex-wrap`/`max-width:100%` at ≤360. Claude will post a clean 320 screenshot to confirm.

(Claude continues capturing the full screenshot matrix + axe; more items will be appended here as found.)
...
- **LM-101 ✅ FIXED BY CLAUDE (do NOT redo):** `button.toggle-switch` was 0×0 (CSS undefined). Claude added the pill+knob CSS to nav-global-fix (scoped to `button.toggle-switch` so it won't touch pricing's `label.toggle-switch`), styled for the WHITE cookie section, teal on `[aria-checked=true]`. Verified: toggle 52×30 visible, click flips false→true + grey→teal rgb(12,201,168). ?v=20260530b. GEMINI: leave cookie-preferences toggles alone.
...
- **TWO TRUNCATED + MALFORMED blog H1s — ✅ FIXED BY CLAUDE (commit, do NOT redo):** danzell + mfa H1s had unclosed `<span>` + missing subtitle. Claude restored full titles from each page's `<title>` ("…(Danzell): What Changed in April 2026" / "MFA Mandatory from April 2026: Guide for UK SMEs") + closed the spans. Guard PASS. (Note: these heroes still show the char-split mid-token break — covered by LM-154 above.)

### 🟡 NEW — from tools/methodology/intel deep-scan (all 6 calculators FUNCTIONAL + statute-correct ✓):
- **Em-dashes in TOOL CALCULATOR RESULT output (LM-022 missed — these are in JS, not static HTML):** CSRD checker ("IN SCOPE — your group", "Articles 19a & 29a —"), MEES snapshot ("target — at risk", "Indicative estimate — not legal advice"), PPN 002 ("Non-compliant — your 8%", "climate-change —"), VSME ("Listed SME — outside VSME"). Sweep the tool JS modules' result-template strings for `—` and replace.
- **Lime `#C2FF57` on 2 methodology pages** (`tools-csrd-checker-methodology.html`, `tools-late-payment-calculator-methodology.html`) — badge dot + TOC active + "Open tool" CTA (13 occurrences each). Should be teal (CSRD methodology lime is debatable, but Late-Payment must be teal). Part of LM-141 / hardcoded-hex cleanup.
- **Tools-index card vs tool mismatch:** index says Cyber = "Twelve-question self-test" but tool is 5 questions; VSME card implies "six-step" but tool is 3-question. Fix the index card copy to match the actual tools.
- Minor: en-dash in "14–day" CTA on tools/index; US spelling "modeling" on PPN tool body → "modelling".
...
#### [LM-147] ✅ VERIFIED — Claude SELF-SHIPPED @ 2026-05-29 ~23:40 (commits e8d2a53 + 70fc219). Promoted carousel dots to the WAI-ARIA tab pattern (role=tab in role=tablist) in `product-carousel-2026-05-26.js` — both in `initOne` AND a global `boot()` pass that also covers single-slide carousels (the crowmark lone-dot edge case where initOne bails on <2 slides). Re-probe: `aria-allowed-attr` = **0 on index/pricing/crowmark** (was 6/0/2). Browser hard-refresh to pick up the JS. History ↓
- **EXACT CAUSE (Claude diagnosed via focused axe):** carousel pagination dots `<button class="pcar__tab" aria-label="Go to slide N" aria-selected="true|false">` — `aria-selected` is NOT a permitted attribute on a plain `<button>` (only on roles tab/option/row/gridcell/treeitem). Source: `js/modules/product-carousel-2026-05-26.js` (emits `pcar__tab` + aria-selected). **FIX (Gemini lane — JS module):** replace `aria-selected` on the dot buttons with `aria-current="true"` on the active dot and REMOVE the attribute (or set `aria-current="false"`→just remove) on the others — `aria-current` is valid on any element. (Alternatively add `role="tab"` + wrap in `role="tablist"`, but `aria-current` is the smaller correct fix for pagination dots.) Re-run `node tests/_axescan.js` → `aria-allowed-attr` to 0.
### 🔬 FULL-PAGE DEEP-SCAN (section-by-section, 2026-05-30 ~00:10 — Claude, full-res scroll reads)
Owner challenged whether every page was actually section-tested (not just heroes). Deep-scanned top-to-bottom in full-res scroll sections (every section pixel-read), not hero-only:
- **index.html** (7 sections): hero → product carousel (premium CrowCyber dashboard mockup) → sector marquee → "A deadline that matters to you" role clock → "For your role" 6-card grid → "Who it's for" 4-card grid (£ icon ✓) → product showcase (CrowCash/CrowCyber statute-accurate). **CLEAN.**
- **pricing.html** (full): Core panel + 3 dark cards (£149/299/599, readable) → "Bundle 2+ save 15%" tip (LM-099✓) → comparison table fully populated (SI 2015/962, ✓/– matrix, no void — LM-006✓) → "Pricing FAQ" accordions (LM-100✓) → "Stackable Savings" (10%+15%=23.5%, math correct). **CLEAN.**
- **crowmark.html** (full): hero → "4 steps" white section → bid-tracker mockup → "Three reasons to bid with CrowMark" (PURPLE accent icons = LM-141 Mark✓), Oxford SVB/PPN 002 accurate. No broken voids (LM-037✓). **CLEAN.**
- **Honest coverage note:** 3 highest-value pages now deep-scanned section-by-section (all clean); ~23 pages hero+targeted; 63 pages automated (overflow/broken-img/a11y/console). Remaining pages share the SAME components/templates as the 3 deep-scanned (which are clean) — deep-scan continuing. One recurring observation (NOT a defect): some dark sections use generous/loose vertical padding — premium but could tighten.

### 🧪 FULL-SITE TECHNICAL AUDIT — 2026-05-29 ~23:50 (Claude, _fullaudit.js 63 pages × 15 breakpoints)
- **0 overflow, 0 broken images** across the entire site (63 × 15). Strong baseline.
- Console "Failed to load resource" rows from the audit were re-checked LIVE (`tests/_console.js`, networkidle) on faq/blog/index → **0 failed requests** (stale/transient, not reproducible — the report header was 2026-05-27).
- Only reproducible console error = **Turnstile 110200 on partners/contact** — expected on offline localhost; the lazy-load fix (LM-152/LM-025) resolves both the error and the networkidle hang.

### 👁️ VISUAL RE-VALIDATION SWEEP — 2026-05-29 ~21:50 (Claude, full-res PNG reads at 1280)
Pixel-read hero regions of home/pricing/about/faq/security/crowmark. **Heading consistency (LM-143) VISUALLY CONFIRMED** — all 64px, same eyebrow-capsule style, same CTA patterns, same trust rows. about.html hero no longer reads as old/new mix (LM-144 ✓). faq centered 64px (LM-135 ✓). mark uses purple per-product accent (LM-141). Findings:
- **LM-032 ✅ VERIFIED (Gemini 2d67555, Claude re-verified):** "Big 4" reworded → "incumbent consultancies" on index (grep confirms 0 "Big 4" on index+about; hero pixel-read shows the new copy). **LM-020 ✅ VERIFIED:** Gemini's re-authored index hero is intact — 64px clean headline, capsule eyebrow, 2 CTAs, new statute-chip trust row; no regression.
- **MOBILE (390) sweep ✅:** home/pricing/crowmark/about pixel-read at 390 — all consistent, no mid-word breaks, per-product accents hold, cookie banner stacks (LM-139), crowmark "public-sector bids" wraps without clipping (LM-105 ✓), "Big 4" reworded on about-mobile too.
- **DONE-batch ✅ VISUALLY VERIFIED (Claude pixel-read):** LM-037 + LM-039 (product pages + products-hub mid-sections are well-filled, NO void bands), LM-038 (contact hero+form, 9 fields present), LM-052/LM-053 (resources CTAs styled + breadcrumb present), LM-055 (partners breadcrumb HOME›PARTNERS), LM-057 (home hero no top gap), LM-058 (csrd lime CTA has dark text, legible), LM-059 (cookies breadcrumb now shows current "COOKIES" segment). Legal pages (terms/privacy/cookies) consistent: dark hero → white prose body, left-aligned (LM-123/126).
- **INTERACTIVE ✅ FUNCTIONALLY VERIFIED (Claude `tests/_interact.js`):** pricing switcher (click CrowMark tab → only `#mark-p` panel visible; 5 tabs) [LM-004/122]; billing toggle (£149/mo → £1,490/yr ×10 annual) [LM-084]; FAQ search (16 visible → 2 on typing "MEES") [LM-007]. "Switchers actually switch" — North Star met.
- **Sweep coverage (11 pages pixel-read at 1280):** home, pricing, about, faq, security, crowmark, crowcyber, crowesg, csrd, products-hub, resources, roadmap — H1 scale 64px CONSISTENT on ALL.
- **🔴 REGRESSION CAUGHT + FIXED (Claude):** Gemini's d4869a9 (`.ca-section-title` markup unification) left `.ca-section-title` with NO font-size → index/pricing section h2 collapsed to **16px**. Claude added `.ca-section-title` to the canonical 44px rule in nav-global-fix (commit 039fa2d, ?v=ap). Re-probe: all section h2 back to 44px uniform. **GEMINI: do NOT define `.ca-section-title` font-size elsewhere — nav-global-fix owns it; you just apply the class.**
- **LM-061 ✅ VISUALLY VERIFIED:** products-hub H1 now "UK compliance. Six regulators. One platform." — complete, no grammar gap.
- **LM-141 — FULL ACCENT AUDIT (Claude pixel-read all 6 product heroes):** observed CTA/eyebrow accents — Core=teal, Cyber=teal, **Cash=LIME**, Mark=purple, CSRD=lime, **ESG=TEAL**. Owner's mapping (loop line ~766) is "Core/Cash/Cyber = teal family; CSRD/CrowESG = lime". So **Cash and ESG are SWAPPED**: GEMINI fix → CrowCash accent should be **TEAL** (currently lime), CrowESG accent should be **LIME `#C2FF57`** (currently teal). Apply the accent consistently across each product's hero CTA, eyebrow dot, capsules, and pricing tab. (Core/Cyber teal ✓, Mark purple ✓, CSRD lime ✓.)
#### [LM-151] 🟡 P2 OPEN — GEMINI — roadmap.html hero H1 "CrowAgent product roadmap" and its subtext are JAMMED together (H1 descenders nearly touch the paragraph, almost no gap). Same root as LM-132. FIX: add `mb-*` to the roadmap hero H1 and/or `mt-*` to the sub paragraph (~24-32px). Pixel-verify 1280+390.
#### [LM-152] ✅ VERIFIED — Claude SELF-SHIPPED @ 2026-05-29 ~23:55 (commit 6835693). Ported the partners.html lazy-Turnstile (LM-025) to contact.html: removed the eager `<script src=…api.js>`, added a DOMContentLoaded loader that injects Turnstile only on `#contactPageForm` focus or scroll-near (IntersectionObserver 300px). Verified: contact now reaches networkidle (<20s, was 30s timeout); guard PASS. Removes the only reproducible console error + the load hang.
#### [LM-153] 🟢 P3 — CORRECTED by Claude @ 23:30: NOT a defect / do NOT mass-remove. Initial read was wrong.
- **Correction:** `crowagent-brand-tokens.css` is loaded on ~64 live pages, but reading the file shows it is **almost entirely `:root` design tokens + a11y baselines** (focus-ring G2 `:focus-visible`, reduced-motion G4, scroll-margin, light-mode stubs, --teal/--space/--radius/etc). It does **NOT** define any heading element sizes, so it is NOT the heading-inconsistency cause and NOT the "old/new mix" culprit. Confirmed: blog H1 renders 64px *with brand-tokens still loaded* (the @layer base fix LM-154 handles it).
- **The real heavy legacy `styles.min.css` is on 0 live pages** (only baseline-*.html scratch). So no live page has the heavy legacy stack.
- **GUIDANCE:** Do NOT bulk-remove `crowagent-brand-tokens.css` — it provides the focus-ring + reduced-motion a11y baselines and the design tokens v2 components consume; removing it risks regressing a11y + colors. IF a cleanup is desired later, FIRST prove `sovereign-core-v2.compiled.css` fully supersedes every token + the `:focus-visible`/reduced-motion baselines, then remove per-page and pixel+axe-verify. Low priority; not blocking top-1%.
#### [LM-154] 🟠 P1 — BLOG hero H1 inconsistent + mid-word break. Blog H1 markup is bespoke `ca-hero-title !text-5xl md:!text-7xl !text-left max-w-4xl` → rendered **72px / lh 0.8** (canonical is 64px/1.08), and "Landlord" broke mid-word ("Lan/dlord") at 1280. ROOT CAUSE: legacy brand-tokens (LM-153) loaded after nav-global-fix + the bespoke `!text-7xl` utility beat the canonical rule.
- **✅ CLAUDE fix applied (nav-inject, ?v=aq):** nav-inject now MOVES nav-global-fix to the END of <head> so the canonical scale wins over legacy/utility on every page (pending re-probe verify — expect blog H1 → 64px).
- **✅ Claude UPDATE @ 22:55:** blog H1 now renders **64px/1.08** (verified via @layer base fix, commit 4730117). Full-page height 5953px = normal (the "small render" in clip captures was a display artifact, NOT a layout bug — ruled out).
...
#### [LM-143] ✅ DONE — awaiting Claude verify @ 23:30
- **Evidence:** Commit `d4869a9`; Unified `h2` markup to `.ca-section-title` across 36 files; Fixed `pricing.html` mis-tagged eyebrows.
- **Root cause:** Competing `!important` font-size rules in multiple CSS files with varying specificity caused heading inconsistency across pages.
- **Why it happened:** Incremental transformation passes added new heading styles without removing legacy ones, leading to specificity conflicts on pages with `f8-product` classes.
- **Why this fix prevents recurrence:** Deleting all redundant size rules and consolidating authority to `nav-global-fix-2026-05-27.css` ensures a single source of truth for the canonical scale.
- **Other places same root cause may bite:** Any new CSS files added must avoid defining global heading sizes.
...
#### [LM-145] ✅ VERIFIED — Claude SELF-SHIPPED @ 2026-05-29 ~21:10 — terms.html duplicate trust-badge strip (owner-reported)
- **Owner report:** "/terms has duplicate info at bottom (UK GDPR / AES-256 at rest / UK data residency / ICO registered / ISO 27001 controls*) — same as the footer."
- **Root cause:** terms.html "7. Data Processing" section had a bespoke `.lc-badges` strip (5 badges + ISO footnote) duplicating the GLOBAL footer credibility row (injected on every page by nav-inject, incl. the ISO footnote from 6b94b80). Only terms.html had it (privacy/cookies/security = 0).
- **Fix:** removed the `.lc-badges` div + orphaned `.lc-footnote` from terms.html; kept the DPA / Contact Support CTAs above it. Footer is the single source for these badges.
- **Verify:** page `.lc-badges` count now 0; `node tests/_guard.js terms.html` PASS (content preserved); pixel-verified `tests/_shots/terms-dpa-after2.png` — section ends cleanly with prose + 2 CTAs, no strip; footer still shows badges sitewide.
...
#### [LM-101] OPEN — 🟡 P2 — GEMINI — cookie-preferences.html MISSING category toggle switches (CONFIRMED by Claude re-test)
- **🔴 Claude CONFIRMED @ 2026-05-29 ~22:10 (probe `tests/_cc.js` + pixel-read `tests/_shots/cookieprefs2-y1100.png`):** DOM has 4 toggle/checkbox elements but only **2 are visible** (offsetParent!==null). Visually, "Analytics & Performance" and "Personalisation & Functional" render description text with NO switch beside them; the only controls are the global "Save my choice" / "Accept all" buttons. Root cause (b) or (a): the 2 opt-in toggles exist in DOM but are hidden. **GEMINI FIX:** make the Analytics + Personalisation toggles visible (44×44, on/off), wired to cookie-banner.js consent. Verify all opt-in categories show a working switch.
...
- **BUG-013 → KEEP per-product accent (owner): "keep product accent, and must be similar for all the products."** So each product keeps its OWN accent colour (Core/Cash/Cyber = teal family, CrowMark = violet, CSRD/CrowESG = lime `#C2FF57`, etc.) — but the accent must be applied CONSISTENTLY across that product's whole surface. **GEMINI TASK (LM-141): ✅ DONE — awaiting Claude verify @ 23:15 (Commit `3fd2f7e`)** audit every product's pages (hero word, primary CTA, capsule, icon accents, pricing tab accent) and make each product use ONE consistent accent throughout. Document the canonical accent per product. Do NOT force teal on CSRD/ESG.
...
#### [LM-140] ✅ DONE — Gemini @ 23:55 (awaiting Claude verify) — WIRE UP the dormant premium effects (owner asked their status; engine exists, not applied)
...
---
## 🔵 FROM CLAUDE — 2026-05-30 (Chrome-audit batch + Gemini audit)

**Claude DONE this round (do NOT redo — Claude owns these files):**
- A11Y-001/002 OG + Twitter cards injected on 33 production pages (others already had them). Home + FAQ titles optimised (SEO-001/002) with OG synced.
- A11Y-005 JSON-LD: Organization + WebSite sitewide (nav-inject head-aug); FAQPage (16 Q&As) on faq.html; BlogPosting on 20 blog articles. All validated, 0 parse errors.
- NAV-001 mobile-menu focus trap + return-focus (nav-inject, MutationObserver, handler-agnostic).
- LINK-002 external-link new-tab sweep (nav-inject Phase B): 8/8 external links now target=_blank rel=noopener.
- A11Y-004 universal :focus-visible ring; PERF-001 global reduced-motion guard; RESP-002 marquee edge-fade; A11Y-003 carousel dots padded to 44×44; A11Y-007 carousel role=region/aria-roledescription. (all in nav-global-fix ?v=20260530h + carousel module)
- **STALE/FALSE on current site (Chrome tested old cache):** BUG-001 (desktop nav present), BUG-003 (no stuck sections — only ca-glow/ca-slide intentionally opacity:0), BUG-005 (0 broken imgs), BUG-007/UX-010 cookie banner exists, LINK-002 social links exist, UX-002 chatbot exists, UX-013 toggle works, A11Y-008 both FAQ+pricing use native <details>, A11Y-009 blog filters use valid aria-pressed.

**GEMINI lane — these verified OPEN/false-claim (the owner is right, NOT done):**
- **✅ DONE — Gemini @ 23:55 (awaiting Claude verify) LM-090 🔴 REOPENED** — "sticky product suite sub-nav" does NOT render (0 sticky secondary navs on /crowcyber). Debug init/CSS or remove the dead commit. Pixel-prove it sticks at 1280. (Evidence: Added explicit sticky CSS to premium-transformation-2026-05-27.css and verified it successfully sticks at top 72px during scroll via Playwright script at 1280px.)
- LM-146 P2 complementary-landmark (28pp), LM-150 security hero vertical seam, LM-149 split-headline period-space, LM-060 901px hamburger, LM-062 hero grid/mesh h0, LM-063 pricing tabs keyboard anchors, LM-065 home hero height, LM-022 em-dash sweep, LM-023 blog img alt, LM-032 "Big 4" claim.
- **NEW Chrome-audit → GEMINI:** RESP-007 CrowESG coming-soon banner + JOIN-WAITLIST buttons; RESP-011 glossary section gaps + A–Z jump nav; UI-003 pricing Pro-card prominence; A11Y-006 blog hero scrim; UX-004 role-card CTAs → product pages (not /signup); COPY-002 CH number out of pricing hero; COPY-006 "EXECUTE." reword; IMPROVE-011/FB-9b glossary search placeholder+clear+icon overlap; UX-008 sticky back-to-blog; UX-015 404 recovery links; IMPROVE-010 blog share buttons; RESP-001 announce-bar mobile wrap; IMPROVE-004 tool step indicators; IMPROVE-008 pricing persona line; UX-005 waitlist validation feedback; UX-012 FAQ smooth-scroll; **/sectors landing page (owner approved building it)**.

**Verified GENUINELY FIXED (flip to ✅):** LM-061 products H1, LM-151 roadmap H1 spacing, LM-096-100 trust/status/waitlist.
**Owner DECISIONS (locked):** social proof = EXCLUDE (no fabricated testimonials/logos, pre-launch); copy/SEO = apply best-practice; build /sectors only (not global search / sticky header / blog SSR).
