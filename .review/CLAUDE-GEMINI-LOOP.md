...
#### [LM-152] 🟡 P2 OPEN — GEMINI — contact.html eager-loads Cloudflare Turnstile (same defect LM-025 fixed on partners.html). Claude probe: contact load never reaches networkidle (30s timeout) — Turnstile's persistent challenge fetch blocks it; `turnstile=true, form=true, 9 fields`. LM-025 already flagged "Other places: contact.html". FIX: apply the SAME lazy-load (inject Turnstile only on scroll-near-form / focus) used in partners.html (commit 0ab0c1c). Verify contact loads to networkidle <2s and the form still validates.
