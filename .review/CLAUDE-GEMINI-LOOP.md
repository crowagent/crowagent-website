...
#### [LM-153] 🟠 P1 OPEN — GEMINI — BLOG pages load LEGACY `crowagent-brand-tokens.css` (old/new mix, like about.html LM-144). Found in `blog/mees-band-c-2028.html` <head> line 37 (loads AFTER nav-global-fix line 36 → legacy file overrides canonical heading scale). Almost certainly on ALL 20 blog posts. FIX (Gemini): remove the `crowagent-brand-tokens.css` <link> from every blog/*.html <head> (migrate any token it provides into the v2 stack first if needed). Verify blogs load 0 legacy stylesheets + guard PASS + word count preserved.
...
- **GEMINI follow-through:** replace the bespoke `!text-5xl md:!text-7xl !text-left` on blog hero H1s with the canonical `.ca-hero-title` (no size utility), and ensure the word-aware char-split (LM-130) applies so single words never break. Pixel-verify a few blog posts at 1280+390.
...
#### [LM-148] 🟡 P2 OPEN — GEMINI — a11y misc (axe). **EXACT CAUSES (Claude diagnosed):** (a) `<div id="scroll-progress" class="scroll-progress" role="progressbar">` on index has NO accessible name — source `js/modules/cinematic-init.js`; **FIX:** add `aria-label="Page scroll progress"` (and ideally `aria-valuemin="0" aria-valuemax="100"` + update `aria-valuenow` on scroll). (b) `label` critical on pricing.html — a form control with no associated `<label>`; add a `<label for=…>` or `aria-label`. (c) residual `color-contrast` (158 nodes, pricing heaviest 17) — pixel-verify real vs alpha-bg false-positives before changing (many are false per prior measurement of 16:1 chips). Re-run axescan after.
