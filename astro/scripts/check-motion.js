...
const ALLOWED = new Map([
  ...
  ['timeline  src/styles/tokens.css  animation-timeline:',
   'DEBT: the [data-field] ambient parallax in tokens.css. A scroll-driven timeline outside motion.css, which is what this rule forbids. It is not a second arrival — it moves an aria-hidden blurred field and never content. This reason USED TO SAY it "paints NOTHING today, because both fields resolve from --orb-violet and --orb-teal and the owner set both to `none` on 2026-08-03", which was false from later the same day: the owner reversed that ("reapply all the gradients back") and tokens.css restored both orbs to live radial-gradients. So this is a live effect on the page, not a timeline driving an invisible object, and the debt is correspondingly less comfortable. It belongs in motion.css with the rest of the scroll-driven work; ADR 0004 names the move. Not done in this pass because tokens.css was being edited by another agent'],
  ...
  ['entrance  src/pages/about.astro  @keyframes chip-in',
   'DEBT, REDUCED 2026-08-03. It was `from { opacity: 0 }` on the four role chips, staggered 0.1 of a beat apart: content animated from FULLY INVISIBLE, the pattern that left 4 of 9 legacy homepage sections permanently hidden and then failed the axe gate a second time in the rebuild. The floor is now 0.35, the same value styles/motion.css uses for every arrival on the site, so nothing is ever hidden and axe has nothing to sample. WHAT REMAINS IS DUPLICATION, NOT A DEFECT: this is still a component-local entrance and motion.css owns arrival. It survives because these chips sit above the fold, where the central arrival correctly does nothing — so deleting it would remove the sequence rather than centralise it, and that is a design decision for the owner rather than a fix'],
  ...
  ['observer  src/components/sections/ReasoningTrace.astro  IntersectionObserver',
   'DEBT: a FOURTH observer, and a different one — not the data-lit trigger but a private count-up on the worked sum, ReasoningTrace.astro:371-395. It is carefully built: it reads its target from a data attribute the template writes, so the number can never disagree with the markup and nothing is ever parsed back out of formatted text, it returns early under reduced motion, and if it never runs the figure is already correct. Two things are still wrong with it. It is a second observer on the SAME route as motion.ts, which already has one and could carry this. And a count-up is the primitive motion.ts deleted rather than ported, on the grounds that MarketShape.astro forbids one on its four figures BY NAME because the legacy build animated them from zero. The same argument reaches this figure'],
]);
