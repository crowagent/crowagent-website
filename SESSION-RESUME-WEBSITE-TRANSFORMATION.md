...
- **Gemini "forensic audit" — verified each against code (not trusted blindly):** #6 btn-ghost leak = FALSE (only `cz-btn-ghost`/mocks); #5 fragmented nav/two-click = FALSE (handlers only in nav-inject.js; opens in 1 click, verified); #1 JTBD vs Bento = design choice, not a bug; #4 inline CSS = NOT debt (10.0KB above-the-fold critical CSS < 14KB budget = web.dev best practice, KEEP); #3 geometric-truth validator = REAL test-only, FIXED (selectors → .cz-hero-title/.cz-hero-btns/.cz-btn-primary/#hero-canvas; now GREEN); #2 hex drift = REAL, fixed below.
- **DESIGN-TOKEN REMEDIATION (Gemini #2, root-cause not patch):** added canonical `--bg-deep:#000212`; migrated ALL 247 value-position brand hexes (#0CC9A8/#040E1A/#000212/#A78BFA/#E8F0FA) → var(--teal/bg/bg-deep/mark/cloud) across 15 hand-authored CSS files via `tools/token-migrate.js` (value-position only, never Tailwind escaped-class selectors). LOSSLESS: tokens==literals, static pages 0px diff, tokens resolve exactly, homepage hero/globe/holo intact. `tools/token-guard.js` enforces at --max=0, wired into `.git/hooks/pre-commit`. **HONEST REMAINING:** the bulk of Gemini's "898" lives in COMPILED Tailwind arbitrary values (`bg-[#0CC9A8]` in HTML → sovereign-core-v2.compiled.css); tokenising those needs Tailwind theme config + HTML class changes across 60+ files = separate Stage-3 effort, NOT yet done. **Stage-3 (patch→source migration of nav-global-fix overrides) also pending.**
- **LEGAL a11y (real defect, was dismissed as false-positive earlier — owner was right):** privacy "What this policy covers" + security "Operational standards" were #040E1A on #000212 = 1.06 contrast (invisible). Root cause: global `.ca-section-light h2{color:var(--bg)!important}` clobbering dark bands nested in light wrapper. Fixed in premium-gloss with `.pv-band:not(--light)/.sec-band:not(--light)` white-heading exception → 20.64 contrast.
