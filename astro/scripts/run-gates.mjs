...
const GATES = [
  ...
  /* Local certification only, deliberately NOT in `build:deploy`. They drive a
     browser, so they belong with the other browser gates on a developer
     machine rather than in the Cloudflare chain. */
  { name: 'check-app-contrast', cmd: node, args: ['scripts/check-app-contrast.js'] },
  { name: 'check-sticky-budget', cmd: node, args: ['scripts/check-sticky-budget.js'] },
