...
| `color-microsoft-entra-id.svg` | Microsoft — Entra ID | Original download URL UNRECORDED. Geometry and the six fills (`#6df`, `#cbf8ff`, `#0294e4`, `#96bcc2`, `#225086`, `#074793`) match `Microsoft Entra ID color icon.svg` in Microsoft's [Entra architecture icons](https://learn.microsoft.com/en-us/entra/architecture/architecture-icons) pack. | Verified against the pack 2026-08-04 | None held. That pack's terms permit "architectural diagrams, training materials, or documentation" and say "Don't use Microsoft product icons in marketing communications." | ⛔ **WITHDRAWN 2026-08-04 — withdrawal 7, by owner decision on A-37.** The last Microsoft mark still served, and the only one withdrawn while being the vendor's correct current artwork — which is the point: correct is not the same as licensed. A Microsoft licence is generally required, the symbol and the logotype must be used together where this page used the symbol alone, and the pack this came from excludes marketing use in terms. The decision was explicitly NOT to seek a licence: near-zero conversion value, an open UK IPO dispute, and a second file of unlicensed third-party marks is the last thing that should be discoverable. Named in text. |
...
| `color-google-drive.svg` | Google — Drive | Original download URL UNRECORDED. | — | None held. Google product icons require permission via the Partner Marketing Hub. | ⛔ **WITHDRAWN 2026-08-04 — withdrawal 8, by owner decision on A-37.** The last Google mark still served. Unlike the corporate "G" of withdrawal 3 there was no guidance sentence forbidding this one outright, and it goes anyway: marketing use of a Google product mark wants a co-branding arrangement through the Partner Marketing Hub and none is held. Permission was deliberately not requested, for the same reasons recorded against Entra ID above. Named in text. |
