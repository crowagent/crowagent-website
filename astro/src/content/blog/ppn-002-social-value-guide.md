...
Proxy values change between editions, and the edition your buyer names is the one that counts. We do not republish them here, because a proxy value quoted from a guide and not from the buyer's stated edition is exactly the sort of figure that falls apart under scrutiny. Our own [PPN 002 calculator](/tools/ppn-002-calculator) does not implement the TOMs measure set or its proxy values either: it checks a weighting against the 10% floor and stops, and its [methodology](/tools/ppn-002-calculator/methodology) says so.
