...
> **RESTORED 2026-09-08.** This file was deleted on 2026-09-07 by commit `d0f89f43`, "delete the
> legacy root tree and the two workflows that audited it", as collateral rather than on purpose. It
> is the ONLY record of where each photograph came from and under what licence, and both
> `astro/src/components/blog/heroes.ts` and `scripts/build-blog-photos.mjs` name it in their own
> text as that record, so for a day the code pointed at a file that was not there. Recovered
> verbatim from `d0f89f43^` and extended with the eleventh photograph. See website row A-276.
...
| `clipboard-check-in-a-stockroom` **(new 2026-09-08)** | A person in a dark jacket and lanyard holding a clipboard of ruled forms with a pen, beside stockroom shelving with a bright window behind | `life-after-award-kpis-contract-performance` | https://www.pexels.com/photo/close-up-shot-of-a-person-doing-a-checklist-12234106/ | Daniel Andraski | Pexels Licence |
| `pins-marking-dates-on-a-calendar` **(new 2026-09-08)** | A calendar page marked with four red push pins and one date circled in red pen | `ppn-026-what-has-published-and-what-has-not` | https://www.pexels.com/photo/close-up-photo-of-red-pins-on-a-calendar-9810172/ | Towfiqu barbhuiya | Pexels Licence |
...
- `clipboard-check-in-a-stockroom` **(2026-09-08)** is cut to **1600 x 900 (16:9)** with a centre crop
  from a 6000 x 4000 original, and carries a tonal change: brightness **x1.18**. It is shot into a
  bright window with the subject in shadow against it, which is the same failure mode
  `office-files-on-shelves` had, so it takes the same correction for the same reason, baked into the
  file rather than applied as a CSS filter.
- `pins-marking-dates-on-a-calendar` **(2026-09-08)** is cut to **1600 x 900 (16:9)** with a centre
  crop from a 4501 x 3003 original, and carries **no tonal change**. It is already a high key picture
  and the brightness correction the two above take exists for photographs that close up on a dark
  page, not for one that would blow out under it.
